...
class PostModelTests(TestCase):
    ...
    def test_post_created_date_auto_now_add(self):
        ...
        self.assertAlmostEqual(post.created_date, timezone.now(), delta=timezone.timedelta(seconds=1))
...
class VoteModelTests(TestCase):
    ...
    def test_str_method(self):
        ...
        self.assertEqual(str(vote), f"Vote {vote.id} by {self.user.username} on {self.post.title}")
...
class VoteSubmitTest(TestCase):
    ...
    def test_multiple_votes(self):
        ...
        self.assertTrue(Vote.objects.filter(post=self.post, user=self.user).exists())
        self.assertTrue(Vote.objects.filter(post=self.post, user=new_user).exists())
