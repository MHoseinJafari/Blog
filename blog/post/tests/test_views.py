...
class PostModelViewSetTests(APITestCase):
    ...
    def test_create_post_as_authenticated_user(self):
        ...
        data = {'title': 'New Post', 'content': 'Content of new post', 'visible': True}
    ...
    def test_create_post_as_anonymous_user(self):
        data = {'title': 'Another Post', 'content': 'Content of another post', 'visible': True}
        response = self.client.post(self.post_list_url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_update_post_as_owner(self):
        self.client.login(username='testuser', password='testpassword')
        data = {'title': 'Updated Post', 'content': 'Updated content', 'visible': True}
    ...
    def test_update_post_as_non_owner(self):
        ...
        data = {'title': 'Updated by admin', 'content': 'Updated content by admin', 'visible': True}
        response = self.client.put(self.post_detail_url, data, format='json')
...
class VoteApiViewTests(APITestCase):
    def setUp(self):
        ...
        self.authenticated_client.login(username='testuser', password='testpassword')
    ...
    def test_submit_vote_as_authenticated_user(self):
        ...
        response = self.authenticated_client.post(self.vote_url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['detail'], 'Your vote has been successfully submitted')

        self.post.refresh_from_db()
        self.assertEqual(self.post.vote.count(), 1)  # Assuming there's a `vote_count` field or similar.
